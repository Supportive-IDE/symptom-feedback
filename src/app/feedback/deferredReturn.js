...
export default function DeferredReturn({misconInfo}) {
    ...
    return <>
        <h1>Unreachable code</h1>
            <p>The code beginning on line {lineNumber} will not run because it follows 
            a <CodeInline code="return" /> statement. The Python interpreter will always exit a function 
            at a <CodeInline code="return" />, skipping any code that follows 
            the <CodeInline code="return" /> in the function. If the code following <CodeInline code="return" /> is 
            needed, move it before the <CodeInline code="return" />. Otherwise, delete it or comment 
            it out.</p>
        ...
        it reaches <CodeInline code="return" />.</p>
