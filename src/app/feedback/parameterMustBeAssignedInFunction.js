...
export default function ParamMustBeAssignedInFunction({misconInfo}) {
    ...
    return <>
        ...
        <p>Avoid setting the value of a parameter inside a function, at least before the parameter has been used. This will allow the code that calls the function 
            to set the parameter value, making the function more reusable.
        </p>
        <p>Read the code below:</p>
        <MiniIDE startingCode={['# Before', '', 'def farenheit_to_celsius(f):', '\tf = float(input("Enter the temperature in Farenheit: "))',
                                '\treturn (f - 32) * 5/9', '','print(farenheit_to_celsius(32))'
        ]} />
        <p>The value of <CodeInline code="f" /> is replaced on line 2. This means that the value passed 
        to <CodeInline code="f" /> when the function is called on line 5 is ignored.</p>
        <p>To fix this issue, remove the line of code that replaces <CodeInline code="f" />:</p>
        <MiniIDE startingCode={['# After', '', 'def farenheit_to_celsius(f):',
                                '\treturn (f - 32) * 5/9', '','print(farenheit_to_celsius(32))'
        ]} />
        <p>If the programmer still wants to ask the user to enter a temperature, they can put this code outside the 
